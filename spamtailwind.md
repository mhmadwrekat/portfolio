<!-- // Need To Refactor
// Need To Refactor
// Need To Refactor
// Need To Refactor
// Need To Refactor
// Need To Refactor
import React from "react";
import Home from "../component/Home/Home";
import Image from "next/image";

const index = () => {
  let space = <b className={`text-red-600 md:grid hidden`}>|</b>;

  let themeColor = "#ff1e52";
  const skillsSize = 50;
  const experienceData = [
    {
      title: "MERN Stack Developer",
      Employment_type: "Internship",
      Company_name: "Webtoon for Web Tech",
      Location: "Jordan - Amman",
      Location_type: "Hybrid",
      start_date: "10-2023",
      end_date: "1-2024",
      skills:
        "HTML5 · CSS3 · JavaScript · ReactJS · MongoDB · NodeJs · ExpressJS",
      img: "/assest/img/webtoon.webp",
      id: 3,
    },
    {
      title: "Frontend Developer",
      Employment_type: "Full-time",
      Company_name: "Alzubda",
      Location: "Jordan - Amman",
      Location_type: "On-site",
      start_date: "5-2022",
      end_date: "9-2022",
      skills:
        "Rebuilt alzubda website using Nextjs framework and tailwind Css framework in order to provide and excellent SEO · API integration and decumentaition on postman · Responsible for the deployment of alzubda website on vercel · Github actions pipeline.",
      img: "/assest/img/zubda.webp",
      id: 2,
    },
    {
      title: "Full Stack Developer",
      Employment_type: "Internship",
      Company_name: "Luminus College(ASAC)",
      Location: "Jordan - Amman",
      Location_type: "Remote",
      start_date: "6-2021",
      end_date: "1-2022",
      skills:
        "Front-End Development · MongoDB · Responsive Web Design · JavaScript · React.js · Node.js · Bootstrap · HTML5 · CSS3 · GitHub · Problem Solving · Work under pressure · Tailwind CSS · Next.js",
      img: "/assest/img/asac.webp",
      id: 1,
    },
  ];
  const commentsData = [
    {
      name: "shihab",
      message: "Good Job mohammad",
    },
    {
      name: "raya islam",
      message: "very nice, keep going",
    },
    {
      name: "zaid sallam",
      message: "cool mohammad, i like user interface!",
    },
    {
      name: "motasem ",
      message: "creative",
    },
  ];
  const arrayOfTitles = [
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill={themeColor}
          width={22}
          height={22}
        >
          <g data-name="Layer 2">
            <path
              d="M12 11a4 4 0 1 0-4-4 4 4 0 0 0 4 4zm6 10a1 1 0 0 0 1-1 7 7 0 0 0-14 0 1 1 0 0 0 1 1z"
              data-name="person"
            ></path>
          </g>
        </svg>
      ),
      text: "About Me",
      link: "#aboutMe",
    },
    {
      svg: (
        <svg
          width={22}
          height={22}
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12ZM13.4881 6.44591C13.8882 6.55311 14.1256 6.96437 14.0184 7.36447L11.4302 17.0237C11.323 17.4238 10.9117 17.6613 10.5116 17.5541C10.1115 17.4468 9.8741 17.0356 9.98131 16.6355L12.5695 6.97624C12.6767 6.57614 13.088 6.3387 13.4881 6.44591ZM14.9697 8.46967C15.2626 8.17678 15.7374 8.17678 16.0303 8.46967L16.2387 8.67801C16.874 9.3133 17.4038 9.84308 17.7678 10.3202C18.1521 10.8238 18.4216 11.3559 18.4216 12C18.4216 12.6441 18.1521 13.1762 17.7678 13.6798C17.4038 14.1569 16.874 14.6867 16.2387 15.322L16.0303 15.5303C15.7374 15.8232 15.2626 15.8232 14.9697 15.5303C14.6768 15.2374 14.6768 14.7626 14.9697 14.4697L15.1412 14.2981C15.8229 13.6164 16.2797 13.1574 16.5753 12.7699C16.8577 12.3998 16.9216 12.1843 16.9216 12C16.9216 11.8157 16.8577 11.6002 16.5753 11.2301C16.2797 10.8426 15.8229 10.3836 15.1412 9.70191L14.9697 9.53033C14.6768 9.23744 14.6768 8.76257 14.9697 8.46967ZM7.96986 8.46967C8.26275 8.17678 8.73762 8.17678 9.03052 8.46967C9.32341 8.76257 9.32341 9.23744 9.03052 9.53033L8.85894 9.70191C8.17729 10.3836 7.72052 10.8426 7.42488 11.2301C7.14245 11.6002 7.07861 11.8157 7.07861 12C7.07861 12.1843 7.14245 12.3998 7.42488 12.7699C7.72052 13.1574 8.17729 13.6164 8.85894 14.2981L9.03052 14.4697C9.32341 14.7626 9.32341 15.2374 9.03052 15.5303C8.73762 15.8232 8.26275 15.8232 7.96986 15.5303L7.76151 15.322C7.12617 14.6867 6.59638 14.1569 6.23235 13.6798C5.84811 13.1762 5.57861 12.6441 5.57861 12C5.57861 11.3559 5.84811 10.8238 6.23235 10.3202C6.59638 9.84308 7.12617 9.31331 7.76151 8.67801L7.96986 8.46967Z"
            fill={themeColor}
          />
        </svg>
      ),
      text: "Skills",
      link: "#Skills",
    },
    {
      svg: (
        <svg
          fill={themeColor}
          width={22}
          height={22}
          viewBox="0 0 1024 1024"
          xmlns="http://www.w3.org/2000/svg"
          className="icon"
        >
          <path d="M880 112H144c-17.7 0-32 14.3-32 32v736c0 17.7 14.3 32 32 32h736c17.7 0 32-14.3 32-32V144c0-17.7-14.3-32-32-32zM368 744c0 4.4-3.6 8-8 8h-80c-4.4 0-8-3.6-8-8V280c0-4.4 3.6-8 8-8h80c4.4 0 8 3.6 8 8v464zm192-280c0 4.4-3.6 8-8 8h-80c-4.4 0-8-3.6-8-8V280c0-4.4 3.6-8 8-8h80c4.4 0 8 3.6 8 8v184zm192 72c0 4.4-3.6 8-8 8h-80c-4.4 0-8-3.6-8-8V280c0-4.4 3.6-8 8-8h80c4.4 0 8 3.6 8 8v256z" />
        </svg>
      ),
      text: "Projects",
      link: "#Projects",
    },
    {
      svg: (
        <svg
          fill={themeColor}
          width={22}
          height={22}
          viewBox="0 0 1024 1024"
          xmlns="http://www.w3.org/2000/svg"
          className="icon"
        >
          <path d="M112 880c0 17.7 14.3 32 32 32h736c17.7 0 32-14.3 32-32V460H112v420zm768-696H712v-64c0-4.4-3.6-8-8-8h-56c-4.4 0-8 3.6-8 8v64H384v-64c0-4.4-3.6-8-8-8h-56c-4.4 0-8 3.6-8 8v64H144c-17.7 0-32 14.3-32 32v176h800V216c0-17.7-14.3-32-32-32z" />
        </svg>
      ),
      text: "Experience",
      link: "#Experience",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          fill={themeColor}
          width={22}
          height={22}
          viewBox="0 0 50 50"
        >
          <path d="M 25 4 C 12.316406 4 2 12.972656 2 24 C 2 30.1875 5.335938 36.066406 10.949219 39.839844 C 10.816406 40.890625 10.285156 43.441406 8.183594 46.425781 L 7.078125 47.992188 L 9.054688 48 C 14.484375 48 18.15625 44.671875 19.363281 43.394531 C 21.195313 43.796875 23.089844 44 25 44 C 37.683594 44 48 35.027344 48 24 C 48 12.972656 37.683594 4 25 4 Z"></path>
        </svg>
      ),
      text: "Comment",
      link: "#Comment",
    },
  ];
  const socialMedia = [
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={35}
          height={35}
          viewBox="0 0 48 48"
          className="hover:scale-125 duration-500 ease-in-out cursor-pointer"
        >
          <path
            fill={themeColor}
            d="M24,4C12.954,4,4,12.954,4,24s8.954,20,20,20s20-8.954,20-20S35.046,4,24,4z"
          ></path>
          <path
            fill="#0f172a"
            d="M36,17.12c-0.882,0.391-1.999,0.758-3,0.88c1.018-0.604,2.633-1.862,3-3	c-0.951,0.559-2.671,1.156-3.793,1.372C29.789,13.808,24,14.755,24,20v2c-4,0-7.9-3.047-10.327-6c-2.254,3.807,1.858,6.689,2.327,7	c-0.807-0.025-2.335-0.641-3-1c0,0.016,0,0.036,0,0.057c0,2.367,1.661,3.974,3.912,4.422C16.501,26.592,16,27,14.072,27	c0.626,1.935,3.773,2.958,5.928,3c-2.617,2.029-7.126,2.079-8,1.977c8.989,5.289,22.669,0.513,21.982-12.477	C34.95,18.818,35.342,18.104,36,17.12"
          ></path>
        </svg>
      ),
      link: "https://twitter.com/wrekatmhmad",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={35}
          height={35}
          viewBox="0 0 64 64"
          fill={themeColor}
          className="hover:scale-125 duration-500 ease-in-out cursor-pointer"
        >
          <path d="M 21.580078 7 C 13.541078 7 7 13.544938 7 21.585938 L 7 42.417969 C 7 50.457969 13.544938 57 21.585938 57 L 42.417969 57 C 50.457969 57 57 50.455062 57 42.414062 L 57 21.580078 C 57 13.541078 50.455062 7 42.414062 7 L 21.580078 7 z M 47 15 C 48.104 15 49 15.896 49 17 C 49 18.104 48.104 19 47 19 C 45.896 19 45 18.104 45 17 C 45 15.896 45.896 15 47 15 z M 32 19 C 39.17 19 45 24.83 45 32 C 45 39.17 39.169 45 32 45 C 24.83 45 19 39.169 19 32 C 19 24.831 24.83 19 32 19 z M 32 23 C 27.029 23 23 27.029 23 32 C 23 36.971 27.029 41 32 41 C 36.971 41 41 36.971 41 32 C 41 27.029 36.971 23 32 23 z"></path>
        </svg>
      ),
      link: "https://instagram.com/mhmadwrekatt?igshid=NzZlODBkYWE4Ng==",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={35}
          height={35}
          viewBox="0 0 48 48"
          className="hover:scale-125 duration-500 ease-in-out cursor-pointer"
        >
          <path
            fill={themeColor}
            d="M24,4C13,4,4,13,4,24s9,20,20,20s20-9,20-20S35,4,24,4z"
          ></path>
          <rect width="4" height="15" x="14" y="19" fill="#0f172a"></rect>
          <path
            fill="#0f172a"
            d="M16,17L16,17c-1.2,0-2-0.9-2-2c0-1.1,0.8-2,2-2c1.2,0,2,0.9,2,2C18,16.1,17.2,17,16,17z"
          ></path>
          <path
            fill="#0f172a"
            d="M35,24.5c0-3-2.5-5.5-5.5-5.5c-1.9,0-3.5,0.9-4.5,2.3V19h-4v15h4v-8c0-1.7,1.3-3,3-3s3,1.3,3,3v8h4	C35,34,35,24.9,35,24.5z"
          ></path>
        </svg>
      ),
      link: "https://www.linkedin.com/in/mohammad-alwrekat",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={35}
          height={35}
          viewBox="0 0 24 24"
          fill={themeColor}
          className="hover:scale-125 duration-500 ease-in-out cursor-pointer"
        >
          <path
            fill="#0f172a"
            d="M12 2A10 10 0 1 0 12 22A10 10 0 1 0 12 2Z"
          ></path>
          <path d="M10.9,2.1c-4.6,0.5-8.3,4.2-8.8,8.7c-0.5,4.7,2.2,8.9,6.3,10.5C8.7,21.4,9,21.2,9,20.8v-1.6c0,0-0.4,0.1-0.9,0.1 c-1.4,0-2-1.2-2.1-1.9c-0.1-0.4-0.3-0.7-0.6-1C5.1,16.3,5,16.3,5,16.2C5,16,5.3,16,5.4,16c0.6,0,1.1,0.7,1.3,1c0.5,0.8,1.1,1,1.4,1 c0.4,0,0.7-0.1,0.9-0.2c0.1-0.7,0.4-1.4,1-1.8c-2.3-0.5-4-1.8-4-4c0-1.1,0.5-2.2,1.2-3C7.1,8.8,7,8.3,7,7.6C7,7.2,7,6.6,7.3,6 c0,0,1.4,0,2.8,1.3C10.6,7.1,11.3,7,12,7s1.4,0.1,2,0.3C15.3,6,16.8,6,16.8,6C17,6.6,17,7.2,17,7.6c0,0.8-0.1,1.2-0.2,1.4 c0.7,0.8,1.2,1.8,1.2,3c0,2.2-1.7,3.5-4,4c0.6,0.5,1,1.4,1,2.3v2.6c0,0.3,0.3,0.6,0.7,0.5c3.7-1.5,6.3-5.1,6.3-9.3 C22,6.1,16.9,1.4,10.9,2.1z"></path>
        </svg>
      ),
      link: "https://github.com/mhmadwrekat",
    },
  ];
  const programLanguage = [
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 48 48"
          className="mr-2 md:mr-0"
        >
          <path fill="#E65100" d="M41,5H7l3,34l14,4l14-4L41,5L41,5z"></path>
          <path fill="#FF6D00" d="M24 8L24 39.9 35.2 36.7 37.7 8z"></path>
          <path
            fill="#FFF"
            d="M24,25v-4h8.6l-0.7,11.5L24,35.1v-4.2l4.1-1.4l0.3-4.5H24z M32.9,17l0.3-4H24v4H32.9z"
          ></path>
          <path
            fill="#EEE"
            d="M24,30.9v4.2l-7.9-2.6L15.7,27h4l0.2,2.5L24,30.9z M19.1,17H24v-4h-9.1l0.7,12H24v-4h-4.6L19.1,17z"
          ></path>
        </svg>
      ),
      name: "HTML - 5",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 48 48"
          className="mr-2 md:mr-0"
        >
          <path fill="#0277BD" d="M41,5H7l3,34l14,4l14-4L41,5L41,5z"></path>
          <path fill="#039BE5" d="M24 8L24 39.9 35.2 36.7 37.7 8z"></path>
          <path
            fill="#FFF"
            d="M33.1 13L24 13 24 17 28.9 17 28.6 21 24 21 24 25 28.4 25 28.1 29.5 24 30.9 24 35.1 31.9 32.5 32.6 21 32.6 21z"
          ></path>
          <path
            fill="#EEE"
            d="M24,13v4h-8.9l-0.3-4H24z M19.4,21l0.2,4H24v-4H19.4z M19.8,27h-4l0.3,5.5l7.9,2.6v-4.2l-4.1-1.4L19.8,27z"
          ></path>
        </svg>
      ),
      name: "CSS - 3",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 48 48"
        >
          <path fill="#ffd600" d="M6,42V6h36v36H6z"></path>
          <path
            fill="#000001"
            d="M29.538 32.947c.692 1.124 1.444 2.201 3.037 2.201 1.338 0 2.04-.665 2.04-1.585 0-1.101-.726-1.492-2.198-2.133l-.807-.344c-2.329-.988-3.878-2.226-3.878-4.841 0-2.41 1.845-4.244 4.728-4.244 2.053 0 3.528.711 4.592 2.573l-2.514 1.607c-.553-.988-1.151-1.377-2.078-1.377-.946 0-1.545.597-1.545 1.377 0 .964.6 1.354 1.985 1.951l.807.344C36.452 29.645 38 30.839 38 33.523 38 36.415 35.716 38 32.65 38c-2.999 0-4.702-1.505-5.65-3.368L29.538 32.947zM17.952 33.029c.506.906 1.275 1.603 2.381 1.603 1.058 0 1.667-.418 1.667-2.043V22h3.333v11.101c0 3.367-1.953 4.899-4.805 4.899-2.577 0-4.437-1.746-5.195-3.368L17.952 33.029z"
          ></path>
        </svg>
      ),
      name: "Javascript",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 64 64"
          className="mr-2 md:mr-0"
        >
          <linearGradient
            id="wNc8qMg_PMLZF-NebSkXWa_AU6Wc7r56Fxz_gr1"
            x1="32"
            x2="32"
            y1="54"
            y2="10"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stopColor="#e6abff"></stop>
            <stop offset="1" stopColor="#6dc7ff"></stop>
          </linearGradient>
          <path
            fill="url(#wNc8qMg_PMLZF-NebSkXWa_AU6Wc7r56Fxz_gr1)"
            d="M54,32c0,5.75-2.2,10.98-5.8,14.9l-0.171,0.187L43,40.646V21h-7v10.686L27.663,21H21v22h7	V32.59l14.66,18.66C39.51,53,35.87,54,32,54c-12.15,0-22-9.85-22-22s9.85-22,22-22S54,19.85,54,32z"
          ></path>
          <linearGradient
            id="wNc8qMg_PMLZF-NebSkXWb_AU6Wc7r56Fxz_gr2"
            x1="32"
            x2="32"
            y1="58"
            y2="6"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stopColor="#c822ff"></stop>
            <stop offset="1" stopColor="#1a6dff"></stop>
          </linearGradient>
          <path
            fill="url(#wNc8qMg_PMLZF-NebSkXWb_AU6Wc7r56Fxz_gr2)"
            d="M32,58C17.663,58,6,46.336,6,32S17.663,6,32,6s26,11.664,26,26S46.337,58,32,58z M32,8	C18.767,8,8,18.767,8,32s10.767,24,24,24s24-10.767,24-24S45.233,8,32,8z M41,41.336V23h-3v14.496L26.683,23H23v18h3V26.81	L44.37,50.2c0.771-0.523,1.509-1.097,2.212-1.714L41,41.336z"
          ></path>
        </svg>
      ),
      name: "Next Js",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 48 48"
        >
          <path
            fill="#00acc1"
            d="M24,9.604c-6.4,0-10.4,3.199-12,9.597c2.4-3.199,5.2-4.398,8.4-3.599 c1.826,0.456,3.131,1.781,4.576,3.247C27.328,21.236,30.051,24,36,24c6.4,0,10.4-3.199,12-9.598c-2.4,3.199-5.2,4.399-8.4,3.6 c-1.825-0.456-3.13-1.781-4.575-3.247C32.672,12.367,29.948,9.604,24,9.604L24,9.604z M12,24c-6.4,0-10.4,3.199-12,9.598 c2.4-3.199,5.2-4.399,8.4-3.599c1.825,0.457,3.13,1.781,4.575,3.246c2.353,2.388,5.077,5.152,11.025,5.152 c6.4,0,10.4-3.199,12-9.598c-2.4,3.199-5.2,4.399-8.4,3.599c-1.826-0.456-3.131-1.781-4.576-3.246C20.672,26.764,17.949,24,12,24 L12,24z"
          ></path>
        </svg>
      ),
      name: "Tailwindcss",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 80 80"
        >
          <path
            fill="#8bb7f0"
            d="M46.5,40c0,3.593-2.907,6.5-6.5,6.5s-6.5-2.907-6.5-6.5s2.907-6.5,6.5-6.5S46.5,36.407,46.5,40z"
          ></path>
          <path
            fill="#4e7ab5"
            d="M40,47c-3.859,0-7-3.14-7-7s3.141-7,7-7s7,3.14,7,7S43.859,47,40,47z M40,34c-3.309,0-6,2.691-6,6 s2.691,6,6,6s6-2.691,6-6S43.309,34,40,34z"
          ></path>
          <g>
            <path
              fill="#8bb7f0"
              d="M24.474,75.503c-1.711,0-3.269-0.392-4.632-1.164c-2.28-1.293-3.856-3.521-4.686-6.62 c-0.965-3.607-0.85-8.398,0.332-13.878C5.956,50.562,0.5,45.531,0.5,40c0-5.532,5.455-10.562,14.987-13.84 c-1.182-5.481-1.298-10.272-0.333-13.879c0.83-3.1,2.406-5.328,4.685-6.62c1.362-0.772,2.921-1.164,4.632-1.164 c4.388,0,9.891,2.73,15.531,7.698c5.64-4.963,11.141-7.691,15.524-7.691c1.711,0,3.269,0.392,4.632,1.164 c2.277,1.291,3.854,3.515,4.684,6.611c0.966,3.606,0.85,8.398-0.333,13.88C74.043,29.437,79.5,34.468,79.5,40 s-5.457,10.563-14.991,13.842c1.182,5.481,1.298,10.272,0.332,13.878c-0.83,3.097-2.405,5.321-4.68,6.612 c-1.364,0.772-2.923,1.164-4.633,1.164c-4.386,0-9.889-2.728-15.527-7.689C34.361,72.774,28.859,75.503,24.474,75.503z M19.337,55.006c-0.991,4.717-1.102,8.747-0.316,11.679c0.542,2.026,1.481,3.431,2.794,4.175c0.753,0.427,1.648,0.644,2.659,0.644 c3.394,0,7.862-2.282,12.621-6.437c-2.385-2.402-4.713-5.121-6.928-8.091C26.336,56.56,22.696,55.898,19.337,55.006z M42.906,65.065c4.758,4.151,9.227,6.431,12.621,6.432c1.011,0,1.906-0.217,2.661-0.644c1.309-0.742,2.246-2.144,2.787-4.167 c0.786-2.934,0.676-6.963-0.315-11.679c-3.362,0.892-7.003,1.554-10.833,1.97C47.613,59.947,45.288,62.665,42.906,65.065z M35.566,57.393c1.452,1.776,2.938,3.433,4.434,4.94c1.492-1.506,2.979-3.163,4.43-4.94C42.939,57.464,41.454,57.5,40,57.5 C38.545,57.5,37.059,57.464,35.566,57.393z M32.349,53.179C34.844,53.392,37.417,53.5,40,53.5c2.585,0,5.155-0.108,7.646-0.32 c1.486-2.084,2.903-4.268,4.214-6.495c1.268-2.155,2.461-4.404,3.547-6.686c-1.083-2.276-2.275-4.524-3.546-6.684 c-1.308-2.222-2.725-4.406-4.215-6.495C45.157,26.608,42.587,26.5,40,26.5c-2.588,0-5.159,0.108-7.647,0.321 c-1.49,2.086-2.909,4.27-4.219,6.495c-1.27,2.159-2.462,4.407-3.546,6.686c1.082,2.274,2.274,4.522,3.545,6.684 C29.437,48.901,30.854,51.084,32.349,53.179z M57.491,44.779c-0.706,1.346-1.438,2.666-2.185,3.934 c-0.753,1.282-1.554,2.568-2.386,3.836c2.374-0.366,4.645-0.835,6.771-1.4C59.101,49.1,58.362,46.963,57.491,44.779z M20.304,51.148c2.127,0.564,4.397,1.034,6.768,1.4c-0.837-1.276-1.638-2.563-2.386-3.835c-0.745-1.267-1.478-2.585-2.183-3.931 C21.636,46.956,20.898,49.091,20.304,51.148z M59.816,40c1.528,3.391,2.773,6.743,3.708,9.977C70.926,47.373,75.5,43.574,75.5,40 s-4.574-7.373-11.976-9.977C62.591,33.257,61.345,36.609,59.816,40z M16.472,30.025C9.073,32.628,4.5,36.427,4.5,40 c0,3.574,4.573,7.373,11.974,9.976c0.934-3.235,2.179-6.587,3.705-9.974C18.652,36.614,17.407,33.262,16.472,30.025z M20.303,28.852c0.594,2.054,1.332,4.19,2.2,6.368c0.704-1.342,1.436-2.661,2.184-3.932c0.752-1.279,1.553-2.565,2.388-3.836 C24.701,27.817,22.431,28.287,20.303,28.852z M52.922,27.451c0.837,1.277,1.638,2.563,2.386,3.837 c0.746,1.267,1.479,2.586,2.185,3.932c0.869-2.177,1.607-4.313,2.201-6.369C57.564,28.286,55.294,27.816,52.922,27.451z M24.471,8.497c-1.011,0-1.905,0.216-2.657,0.644c-1.312,0.744-2.252,2.149-2.795,4.175c-0.785,2.935-0.675,6.964,0.317,11.679 c3.36-0.892,7.002-1.555,10.834-1.971c2.217-2.971,4.545-5.688,6.927-8.088C32.337,10.779,27.866,8.497,24.471,8.497z M49.828,23.023c3.83,0.416,7.471,1.078,10.833,1.97c0.992-4.716,1.103-8.746,0.316-11.68c-0.542-2.022-1.48-3.424-2.79-4.166 c-0.754-0.427-1.649-0.644-2.661-0.644c-3.393,0-7.86,2.281-12.619,6.433C45.287,17.335,47.613,20.053,49.828,23.023z M40,22.5 c1.454,0,2.939,0.036,4.431,0.107c-1.45-1.774-2.936-3.43-4.429-4.938c-1.497,1.51-2.982,3.166-4.432,4.938 C37.061,22.536,38.546,22.5,40,22.5z"
            ></path>
            <path
              fill="#4e7ab5"
              d="M24.471,4.997c4.494,0,9.996,2.91,15.532,7.867c5.535-4.953,11.034-7.86,15.524-7.86 c1.623,0,3.099,0.37,4.386,1.099c2.156,1.222,3.652,3.343,4.446,6.306c1.077,3.917,0.851,8.74-0.434,14.08 C73.147,29.57,79,34.431,79,40s-5.853,10.43-15.076,13.512c1.341,5.773,1.566,10.726,0.434,14.079 c-0.794,2.962-2.289,5.084-4.443,6.306C58.626,74.627,57.455,75,55.833,75c-0.001,0-0.416,0-0.417,0 c-4.492,0-9.882-2.91-15.416-7.861c-5.535,4.956-11.035,7.864-15.527,7.864c-1.623,0-3.099-0.37-4.386-1.099 c-2.158-1.225-3.655-3.349-4.448-6.314c-1.19-3.717-0.95-8.603,0.434-14.079C6.852,50.429,1,45.569,1,40 c0-5.568,5.852-10.429,15.072-13.511C15.385,23.43,14.946,20.59,14.931,18c-0.012-2.046,0.267-3.949,0.707-5.59 c0.794-2.966,2.29-5.09,4.448-6.314C21.372,5.366,22.848,4.997,24.471,4.997 M18.958,25.614c3.477-0.955,7.338-1.677,11.483-2.117 c2.382-3.21,4.875-6.096,7.386-8.584c-4.872-4.346-9.616-6.917-13.356-6.917c-1.099,0-2.075,0.238-2.904,0.709 c-1.433,0.813-2.452,2.32-3.031,4.481C17.678,16.388,17.873,20.716,18.958,25.614 M61.039,25.613 c1.085-4.898,1.28-9.227,0.422-12.429c-0.578-2.157-1.597-3.661-3.027-4.472c-0.83-0.471-1.809-0.709-2.907-0.709 c-3.737,0-8.478,2.568-13.349,6.91c2.51,2.488,5,5.374,7.38,8.583C53.701,23.936,57.562,24.658,61.039,25.613 M34.477,23.166 c1.802-0.107,3.609-0.2,5.488-0.2c1.879,0,3.755,0.094,5.557,0.2c-1.808-2.279-3.664-4.361-5.521-6.204 C38.144,18.805,36.287,20.887,34.477,23.166 M40,54c2.761,0,5.404-0.121,7.918-0.342c1.519-2.117,2.988-4.365,4.372-6.719 c1.353-2.299,2.577-4.626,3.67-6.939c-1.093-2.313-2.317-4.639-3.669-6.937c-1.385-2.355-2.853-4.602-4.373-6.719 C45.405,26.121,42.762,26,40,26s-5.406,0.121-7.92,0.343c-1.521,2.117-2.991,4.365-4.376,6.719 c-1.352,2.299-2.576,4.626-3.669,6.939c1.093,2.313,2.316,4.639,3.668,6.937c1.385,2.354,2.854,4.602,4.374,6.719 C34.592,53.879,37.237,54,40,54 M57.546,36.413c1.125-2.709,2.056-5.369,2.766-7.913c-2.553-0.702-5.373-1.282-8.428-1.707 c1.028,1.527,2.029,3.11,2.992,4.748C55.83,33.16,56.717,34.788,57.546,36.413 M22.449,36.414c0.828-1.625,1.716-3.253,2.668-4.873 c0.964-1.638,1.966-3.22,2.995-4.747c-3.055,0.424-5.875,1.005-8.428,1.707C20.394,31.045,21.325,33.704,22.449,36.414 M63.188,50.622C71.264,47.886,76,43.866,76,40s-4.736-7.886-12.812-10.622c-0.953,3.405-2.269,6.983-3.92,10.621 C60.919,43.638,62.235,47.216,63.188,50.622 M16.81,50.621c0.952-3.404,2.267-6.982,3.918-10.619 c-1.651-3.638-2.967-7.217-3.919-10.622C8.734,32.116,4,36.135,4,40C4,43.866,8.735,47.885,16.81,50.621 M51.884,53.207 c3.055-0.424,5.875-1.004,8.428-1.707c-0.711-2.544-1.642-5.204-2.766-7.914c-0.829,1.625-1.717,3.253-2.67,4.873 C53.913,50.097,52.912,51.68,51.884,53.207 M28.109,53.206c-1.028-1.527-2.029-3.109-2.993-4.746 c-0.952-1.619-1.839-3.246-2.667-4.87c-1.124,2.708-2.054,5.367-2.764,7.91C22.237,52.201,25.056,52.781,28.109,53.206 M24.474,72.003c3.739,0,8.481-2.57,13.352-6.914c-2.513-2.489-5.005-5.376-7.388-8.587c-4.143-0.439-8.002-1.161-11.478-2.116 c-1.084,4.898-1.279,9.226-0.422,12.428c0.578,2.16,1.598,3.667,3.03,4.48C22.396,71.765,23.375,72.003,24.474,72.003 M55.527,71.997L55.527,71.997c1.099,0,2.076-0.238,2.907-0.709c1.43-0.811,2.447-2.315,3.024-4.473 c0.858-3.203,0.663-7.531-0.421-12.428c-3.477,0.955-7.337,1.677-11.481,2.116c-2.38,3.21-4.871,6.096-7.381,8.585 C47.046,69.428,51.788,71.996,55.527,71.997 M40,63.04c1.857-1.844,3.713-3.927,5.522-6.206c-1.801,0.107-3.559,0.333-5.439,0.333 c-1.881,0-3.807-0.226-5.609-0.333C36.284,59.114,38.142,61.197,40,63.04 M24.116,4.072c-1.799,0-3.085,0.338-4.523,1.153 c-2.399,1.361-4.055,3.691-4.921,6.926c-0.96,3.587-0.877,8.302,0.236,13.681C5.419,29.189,0,34.325,0,40 c0,5.676,5.42,10.811,14.909,14.168c-1.113,5.379-1.196,10.094-0.235,13.681c0.865,3.233,2.52,5.563,4.921,6.926 c1.44,0.816,3.17,1.163,4.968,1.163c4.487,0,9.753-2.535,15.438-7.466c5.684,4.926,11.039,7.524,15.525,7.525 c1.799,0,3.441-0.413,4.881-1.229c2.397-1.359,4.051-3.686,4.916-6.917c0.961-3.588,0.878-8.303-0.235-13.681 C74.579,50.812,80,45.676,80,40s-5.42-10.812-14.911-14.169c1.114-5.38,1.196-10.095,0.235-13.682 c-0.866-3.231-2.521-5.558-4.919-6.917c-1.439-0.815-3.081-1.229-4.879-1.229c-4.485,0-9.839,2.599-15.524,7.527 c-5.686-4.932-11.066-7.461-15.554-7.461L24.116,4.072z M19.72,24.377c-0.462-2.324-0.772-4.502-0.789-6.412 c-0.015-1.72,0.221-3.217,0.57-4.521c0.507-1.892,1.368-3.194,2.559-3.87c0.676-0.384,1.5-0.644,2.423-0.644 c3.181,0,7.367,2.178,11.883,6.034c-2.221,2.273-4.392,4.819-6.466,7.585C26.315,22.949,22.897,23.562,19.72,24.377L19.72,24.377z M43.638,14.966c4.515-3.853,8.711-5.963,11.888-5.963c0.924,0,1.737,0.195,2.414,0.579c1.189,0.674,2.049,1.973,2.555,3.861 c0.874,2.818,0.761,6.564-0.218,10.933c-3.177-0.814-6.594-1.428-10.178-1.826C48.026,19.785,45.857,17.239,43.638,14.966 L43.638,14.966z M36.673,22.06c1.096-1.3,2.209-2.53,3.328-3.678c1.119,1.148,2.231,2.378,3.326,3.678 C42.208,22.02,41.095,22,40,22S37.793,22.02,36.673,22.06L36.673,22.06z M32.625,27.3c2.405-0.199,4.883-0.341,7.375-0.341 c2.491,0,4.969,0.142,7.373,0.341c1.432,2.019,2.796,4.126,4.056,6.269c1.222,2.078,2.373,4.239,3.423,6.43 c-1.051,2.192-2.202,4.354-3.424,6.432c-1.261,2.144-2.624,4.25-4.056,6.269c-2.403,0.199-4.881,0.466-7.372,0.466 c-2.493,0-4.972-0.268-7.378-0.467c-1.433-2.018-2.797-4.125-4.058-6.269c-1.222-2.077-2.372-4.238-3.422-6.43 c1.05-2.192,2.201-4.354,3.423-6.432C29.827,31.425,31.192,29.318,32.625,27.3L32.625,27.3z M57.422,34.024 c-0.549-1.018-1.112-2.018-1.683-2.99c-0.57-0.969-1.167-1.942-1.785-2.909c1.774,0.302,3.485,0.663,5.114,1.08 C58.601,30.771,58.05,32.383,57.422,34.024L57.422,34.024z M20.928,29.205c1.629-0.417,3.34-0.778,5.114-1.08 c-0.619,0.967-1.216,1.94-1.786,2.908c-0.571,0.972-1.134,1.972-1.683,2.99C21.945,32.384,21.395,30.772,20.928,29.205 L20.928,29.205z M60.365,39.999c1.415-3.162,2.587-6.292,3.49-9.326C70.758,33.192,75,36.721,75,40s-4.242,6.808-11.145,9.327 C62.951,46.292,61.78,43.161,60.365,39.999L60.365,39.999z M16.142,49.326C9.241,46.807,5,43.278,5,40s4.241-6.807,11.141-9.326 c0.903,3.035,2.074,6.165,3.489,9.327C18.216,43.162,17.045,46.292,16.142,49.326L16.142,49.326z M53.953,51.876 c0.618-0.968,1.216-1.941,1.785-2.909c0.572-0.972,1.135-1.973,1.684-2.991c0.628,1.641,1.178,3.254,1.646,4.821 C57.438,51.212,55.727,51.574,53.953,51.876L53.953,51.876z M20.929,50.795c0.467-1.566,1.017-3.177,1.644-4.816 c0.548,1.017,1.111,2.017,1.682,2.988c0.569,0.968,1.166,1.94,1.785,2.908C24.267,51.572,22.557,51.211,20.929,50.795 L20.929,50.795z M24.474,71.003c-0.924,0-1.736-0.195-2.413-0.579c-1.19-0.676-2.051-1.977-2.557-3.869 c-0.831-2.845-0.737-6.592,0.218-10.933c3.176,0.814,6.592,1.428,10.175,1.826c2.075,2.766,4.246,5.313,6.468,7.588 C31.85,68.893,27.653,71.004,24.474,71.003L24.474,71.003L24.474,71.003z M50.098,57.45c3.584-0.399,7.001-1.012,10.178-1.826 c1.105,4.74,1.192,8.518,0.217,10.932c-0.505,1.889-1.364,3.188-2.552,3.861c-0.678,0.384-1.49,0.579-2.413,0.579v1l-0.001-1 c-3.18,0-7.377-2.11-11.891-5.961C45.856,62.762,48.025,60.216,50.098,57.45L50.098,57.45z M40,61.62 c-1.12-1.148-2.234-2.379-3.33-3.68C37.79,57.98,38.904,58,40,58c1.095,0,2.208-0.02,3.327-0.06 C42.232,59.241,41.119,60.472,40,61.62L40,61.62z"
            ></path>
          </g>
        </svg>
      ),
      name: "React Js",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 48 48"
        >
          <path
            fill="#673ab7"
            d="M42,37c0,2.762-2.238,5-5,5H11c-2.761,0-5-2.238-5-5V11c0-2.762,2.239-5,5-5h26c2.762,0,5,2.238,5,5 V37z"
          ></path>
          <path
            fill="#fff"
            d="M33.03,25.6c-0.65-0.9-1.59-1.52-2.8-1.85c0,0,1.02-0.37,1.94-1.75c0.55-0.88,0.83-1.94,0.83-3.18 c0-2.15-0.78-3.8-2.34-4.93C29.1,12.76,27.34,12,24.35,12H15v24h10.43c2.83-0.02,4.96-0.63,6.41-1.8c1.44-1.19,2.16-2.95,2.16-5.3 C34,27.6,33.68,26.5,33.03,25.6z M21,16c0,0,4.17,0,4.25,0c1.52,0,2.75,1.23,2.75,2.75c0,1.52-1.23,2.75-2.75,2.75 c-0.08,0-4.25,0-4.25,0V16z M26,32h-5v-6h5c1.66,0,3,1.34,3,3C29,30.66,27.66,32,26,32z"
          ></path>
        </svg>
      ),
      name: "Bootstrap",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 48 48"
        >
          <path
            fill="#21a366"
            d="M24.007,45.419c-0.574,0-1.143-0.15-1.646-0.44l-5.24-3.103c-0.783-0.438-0.401-0.593-0.143-0.682	c1.044-0.365,1.255-0.448,2.369-1.081c0.117-0.067,0.27-0.043,0.39,0.028l4.026,2.389c0.145,0.079,0.352,0.079,0.486,0l15.697-9.061	c0.145-0.083,0.24-0.251,0.24-0.424V14.932c0-0.181-0.094-0.342-0.243-0.432L24.253,5.446c-0.145-0.086-0.338-0.086-0.483,0	L8.082,14.499c-0.152,0.086-0.249,0.255-0.249,0.428v18.114c0,0.173,0.094,0.338,0.244,0.42l4.299,2.483	c2.334,1.167,3.76-0.208,3.76-1.591V16.476c0-0.255,0.2-0.452,0.456-0.452h1.988c0.248,0,0.452,0.196,0.452,0.452v17.886	c0,3.112-1.697,4.9-4.648,4.9c-0.908,0-1.623,0-3.619-0.982l-4.118-2.373C5.629,35.317,5,34.216,5,33.042V14.928	c0-1.179,0.629-2.279,1.646-2.861L22.36,3.002c0.994-0.562,2.314-0.562,3.301,0l15.694,9.069C42.367,12.656,43,13.753,43,14.932	v18.114c0,1.175-0.633,2.271-1.646,2.861L25.66,44.971c-0.503,0.291-1.073,0.44-1.654,0.44"
          ></path>
          <path
            fill="#21a366"
            d="M28.856,32.937c-6.868,0-8.308-3.153-8.308-5.797c0-0.251,0.203-0.452,0.455-0.452h2.028	c0.224,0,0.413,0.163,0.448,0.384c0.306,2.066,1.218,3.108,5.371,3.108c3.308,0,4.715-0.747,4.715-2.502	c0-1.01-0.401-1.76-5.54-2.263c-4.299-0.424-6.955-1.371-6.955-4.809c0-3.167,2.672-5.053,7.147-5.053	c5.026,0,7.517,1.745,7.831,5.493c0.012,0.13-0.035,0.255-0.122,0.35c-0.086,0.09-0.208,0.145-0.334,0.145h-2.039	c-0.212,0-0.397-0.149-0.44-0.354c-0.491-2.173-1.678-2.868-4.904-2.868c-3.611,0-4.031,1.257-4.031,2.2	c0,1.143,0.495,1.477,5.367,2.122c4.825,0.64,7.116,1.544,7.116,4.935c0,3.418-2.853,5.379-7.827,5.379"
          ></path>
        </svg>
      ),
      name: "Node Js",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 48 48"
        >
          <path
            fill="#5d4037"
            d="M42,17.3C42,37.8,24,44,24,44S6,37.8,6,17.3c0-2.5,0.2-4.6,0.4-6.3c0.3-2.5,2-4.5,4.4-5.1 C13.9,5,18.8,4,24,4s10.1,1,13.3,1.9c2.4,0.6,4.1,2.7,4.4,5.1C41.8,12.7,42,14.9,42,17.3z"
          ></path>
          <path
            fill="#4caf50"
            d="M24,7c4.9,0,9.5,1,12.5,1.8c1.2,0.3,2,1.3,2.2,2.6c0.2,1.9,0.3,3.9,0.3,5.9c0,15.6-11.5,21.9-15,23.4 c-3.5-1.6-15-7.9-15-23.4c0-2,0.1-4,0.3-5.9c0.1-1.3,1-2.3,2.2-2.6C14.5,8,19.1,7,24,7 M24,4c-5.2,0-10.1,1-13.3,1.9 C8.4,6.5,6.6,8.6,6.4,11C6.2,12.7,6,14.9,6,17.3C6,37.8,24,44,24,44s18-6.2,18-26.7c0-2.5-0.2-4.6-0.4-6.3c-0.3-2.5-2-4.5-4.4-5.1 C34.1,5,29.2,4,24,4L24,4z"
          ></path>
          <path fill="#dcedc8" d="M23 28H25V36H23z"></path>
          <path
            fill="#4caf50"
            d="M24,10c0,0-6,5-6,13c0,5.2,3.3,8.5,5,10l1-3l1,3c1.7-1.5,5-4.8,5-10C30,15,24,10,24,10z"
          ></path>
          <path
            fill="#81c784"
            d="M24,10c0,0-6,5-6,13c0,5.2,3.3,8.5,5,10l1-3V10z"
          ></path>
        </svg>
      ),
      name: "MongoDB",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 64 64"
        >
          <linearGradient
            id="Ptgyoqd6DQJu9ZUJIh3sLa_2ZOaTclOqD4q_gr1"
            x1="30.982"
            x2="30.982"
            y1="21.203"
            y2="43.514"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stopColor="#6dc7ff"></stop>
            <stop offset="1" stopColor="#e6abff"></stop>
          </linearGradient>
          <path
            fill="url(#Ptgyoqd6DQJu9ZUJIh3sLa_2ZOaTclOqD4q_gr1)"
            d="M45.758,32.346L56.964,47h-0.85c-1.051,0-2.041-0.49-2.68-1.324L44.5,33.992l-8.935,11.684	C34.927,46.51,33.937,47,32.886,47h-0.85l11.206-14.654L32.271,18h0.85c1.051,0,2.041,0.49,2.68,1.324l8.7,11.377l8.7-11.377	C53.838,18.49,54.829,18,55.879,18h0.85L45.758,32.346z M28.527,40h0.638c0,0,0,0,0,0c-2.348,6.085-9.292,9.631-15.896,7.256	C8.215,45.437,5,40.444,5,35.073L5,29.5c0-7.29,6.273-13.143,13.705-12.443C25.182,17.667,30,23.361,30,29.866L30,34H19v0H7l0,1.109	c0,4.501,2.671,8.705,6.9,10.248c4.807,1.754,9.835-0.235,12.389-4.116C26.789,40.481,27.616,40,28.527,40z M7,32h21v-2.5	C28,23.71,23.29,19,17.5,19S7,23.71,7,29.5V32z"
          ></path>
          <linearGradient
            id="Ptgyoqd6DQJu9ZUJIh3sLb_2ZOaTclOqD4q_gr2"
            x1="44.5"
            x2="44.5"
            y1="16"
            y2="49"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stopColor="#1a6dff"></stop>
            <stop offset="1" stopColor="#c822ff"></stop>
          </linearGradient>
          <path
            fill="url(#Ptgyoqd6DQJu9ZUJIh3sLb_2ZOaTclOqD4q_gr2)"
            d="M56.729,18L45.758,32.347L56.964,47h-0.85	c-1.051,0-2.041-0.49-2.68-1.324L44.5,33.992l-8.935,11.684C34.927,46.51,33.937,47,32.886,47h-0.85l11.206-14.653L32.271,18h0.85	c1.051,0,2.041,0.49,2.68,1.324l8.7,11.377l8.7-11.377C53.838,18.49,54.829,18,55.879,18H56.729 M60.776,16h-4.047h-0.85	c-1.663,0-3.258,0.788-4.268,2.109L44.5,27.408l-7.111-9.299C36.379,16.788,34.783,16,33.121,16h-0.85h-4.047l2.458,3.215	l10.042,13.132L30.447,45.785L27.989,49h4.047h0.85c1.663,0,3.258-0.789,4.268-2.109l7.346-9.606l7.346,9.606	C52.856,48.211,54.451,49,56.114,49h0.85h4.047l-2.458-3.215L48.276,32.346l10.042-13.132L60.776,16L60.776,16z"
          ></path>
          <linearGradient
            id="Ptgyoqd6DQJu9ZUJIh3sLc_2ZOaTclOqD4q_gr3"
            x1="17.54"
            x2="17.54"
            y1="15"
            y2="50"
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0" stopColor="#1a6dff"></stop>
            <stop offset="1" stopColor="#c822ff"></stop>
          </linearGradient>
          <path
            fill="url(#Ptgyoqd6DQJu9ZUJIh3sLc_2ZOaTclOqD4q_gr3)"
            d="M17.5,17c0.397,0,0.8,0.019,1.205,0.057	C25.182,17.667,30,23.361,30,29.866V34H19v0H7v1.109c0,4.501,2.671,8.705,6.9,10.248C15.1,45.796,16.314,46,17.5,46	c3.563,0,6.872-1.847,8.789-4.758C26.789,40.481,27.616,40,28.527,40h0.638c0,0,0,0,0,0c-1.847,4.785-6.535,8-11.665,8	c-1.394,0-2.82-0.237-4.231-0.745C8.215,45.437,5,40.444,5,35.073V29.5C5,22.607,10.607,17,17.5,17 M7,32h21v-2.5	C28,23.71,23.29,19,17.5,19S7,23.71,7,29.5V32 M17.5,15L17.5,15c-3.868,0-7.508,1.509-10.249,4.251C4.51,21.992,3,25.632,3,29.5	v5.573c0,6.348,3.855,12,9.592,14.065C14.184,49.71,15.835,50,17.5,50c5.951,0,11.389-3.729,13.531-9.28l1.05-2.72l-2.915,0	l-0.638,0c-1.565,0-3.026,0.801-3.91,2.142C23.052,42.522,20.324,44,17.5,44c-0.986,0-1.967-0.175-2.915-0.521	C11.509,42.356,9.362,39.416,9.042,36H19l11,0l2,0v-2v-4.134c0-7.607-5.758-14.109-13.108-14.8C18.43,15.022,17.962,15,17.5,15	L17.5,15z M9,30v-0.5c0-4.687,3.813-8.5,8.5-8.5s8.5,3.813,8.5,8.5V30H9L9,30z"
          ></path>
        </svg>
      ),
      name: "Express",
    },
    {
      svg: (
        <svg
          width={skillsSize - 5}
          height={skillsSize - 5}
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
          fill="#039BE5"
        >
          <g>
            <path fill="none" d="M0 0h24v24H0z" />
            <path d="M3 3h18a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1zm4 12.5v-4l2 2 2-2v4h2v-7h-2l-2 2-2-2H5v7h2zm11-3v-4h-2v4h-2l3 3 3-3h-2z" />
          </g>
        </svg>
      ),
      name: "Markdown",
    },
    {
      svg: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width={skillsSize}
          height={skillsSize}
          viewBox="0 0 48 48"
        >
          <path
            fill="#0277BD"
            d="M24.047,5c-1.555,0.005-2.633,0.142-3.936,0.367c-3.848,0.67-4.549,2.077-4.549,4.67V14h9v2H15.22h-4.35c-2.636,0-4.943,1.242-5.674,4.219c-0.826,3.417-0.863,5.557,0,9.125C5.851,32.005,7.294,34,9.931,34h3.632v-5.104c0-2.966,2.686-5.896,5.764-5.896h7.236c2.523,0,5-1.862,5-4.377v-8.586c0-2.439-1.759-4.263-4.218-4.672C27.406,5.359,25.589,4.994,24.047,5z M19.063,9c0.821,0,1.5,0.677,1.5,1.502c0,0.833-0.679,1.498-1.5,1.498c-0.837,0-1.5-0.664-1.5-1.498C17.563,9.68,18.226,9,19.063,9z"
          ></path>
          <path
            fill="#FFC107"
            d="M23.078,43c1.555-0.005,2.633-0.142,3.936-0.367c3.848-0.67,4.549-2.077,4.549-4.67V34h-9v-2h9.343h4.35c2.636,0,4.943-1.242,5.674-4.219c0.826-3.417,0.863-5.557,0-9.125C41.274,15.995,39.831,14,37.194,14h-3.632v5.104c0,2.966-2.686,5.896-5.764,5.896h-7.236c-2.523,0-5,1.862-5,4.377v8.586c0,2.439,1.759,4.263,4.218,4.672C19.719,42.641,21.536,43.006,23.078,43z M28.063,39c-0.821,0-1.5-0.677-1.5-1.502c0-0.833,0.679-1.498,1.5-1.498c0.837,0,1.5,0.664,1.5,1.498C29.563,38.32,28.899,39,28.063,39z"
          ></path>
        </svg>
      ),
      name: "Python",
    },
  ];
  const projectsData = [
    {
      name: "Body Mass",
      status: "RESPONSIVE",
      img: "/assest/img/bodymass.webp",
      link: "https://practice-templates.vercel.app/templates/BodyMass",
      desc: "A website that calculates the body mass index based on height and weight.",
      id: 6,
    },
    {
      name: "Profile Card",
      status: "RESPONSIVE",
      img: "/assest/img/profilecard.webp",
      link: "https://mhmadwrekat.github.io/profile_card/",
      desc: "A website made for displaying profile card and information and projects.",
      id: 5,
    },
    {
      name: "Alzubda",
      status: "RESPONSIVE",
      img: "/assest/img/alzubda.WebP",
      link: "https://alzubda.com",
      desc: "A special news site for Al-zubda News Company during my work for them .",
      id: 4,
    },
    {
      name: "Cella Center",
      status: "RESPONSIVE",
      img: "/assest/img/cella.WebP",
      link: "https://cella-center.vercel.app/",
      desc: "A project For a Beauty Center, Display The Beauty Services and Informations .",
      id: 3,
    },
    {
      name: "Frontend Training",
      status: "RESPONSIVE",
      img: "https://practice-templates.vercel.app/_next/image?url=%2Fassest%2Fimg%2FFrontend_home.webp&w=128&q=75",
      link: "https://practice-templates.vercel.app/",
      desc: "A project to Become a better frontend developer with practice on Mini features.",
      id: 2,
    },
    {
      name: "Portfolio Template",
      status: "DESDKTOP",
      img: "/assest/img/templatePortfolio.webp",
      link: "https://mhmadwrekat.github.io/portfolio-template/",
      desc: "Desktop only Website that Display Template Portfolio, using HTML5, CSS3.",
      id: 1,
    },
  ];
  let open = true;
  return (
    <React.Fragment>
      {/* <link
        rel="stylesheet"
        href="https://unpkg.com/tailwindcss@2.2.19/dist/tailwind.min.css"
      /> */}

      {/*
      <p className="transition duration-700 ease-in-out hover:transition hover:duration-700 duration-500 duration-200 duration-300 hover:ease-in-out opacity-100 opacity-75 opacity-50 opacity-0 opacity-10 relative z-50 z-40 z-30 z-20 z-10 z-0 z-auto border-spotYellow text-center md:text-start flex md:justift-start justify-center gird md:grid-cols-4 md:gap-5 pt-3 bg-gray-900 bg-slate950 bg-blue-200 w-9/12 md:w-12/12 m-auto md:m-0 mt-12 md:mt-16 grid grid-cols-2 w-11/12 md:w-12/12 ml-auto md:m-auto md:m-0 mt-12 md:mt-16 grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5 md:mr-5 md:grid-cols-4 gap-3 md:gap-5 mr-0 md:mr-5 bg-navyBlue text-spotYellow bg-grgay-600 hover:scale-105 hover:scale-100 hover:scale-110 hover:scale-100 hover:rotate-6 text-6xl text-gray-100 text-center my-20 bg-slate-900 hover:skew-y-6 skew-y-12 -skew-y-6 bg-slate-900 hover:-rotate-12 hover:-rotate-6 hover:-rotate-8 -rotate-16 -rotate-6 -rotate-7 -rotate-8 -rotate-9 -rotate-10 -rotate-11 -rotate-12 origin-top-left rotate-6 rotate-12 origin-bottom rotate-12 border-6 rounded rounded-xl bg-spotYellow text-gray-900 font-bold px-4 px-0 px-1 py-1 py-1.5 h-3/12 px-2 px-3 px-5 px-6 px-7 px-8 px-9 px-10 py-2 text-lg text-base w-8/12 w-6/12 w-7/12 w-9/12 text-gray-100 text-gray-200 text-gray-300 text-gray-400 text-gray-500 text-4xl text-5xl rounded opacity-10 w-4/5 pt-3 mx-auto border-b-2 opacity-50 opacity-25 border-gray-100 border-1 lg:mx-0 text-gray-400 bg-gray-400 fill-gray-400 sticky lg:w-10/12 2xl:w-11/12 lg:w-60 lg:w-64 lg:w-72 w-72 mt-0 mt-1 mt-2 mt-3 mt-4 mt-5 mt-6 mt-7 mt-8 mt-9 mt-10 mb-0 mb-1 mb-2 mb-3 mb-4 mb-5 flex ml-0 ml-1 mb-0 mb-1 mb-2 mb-3 ml-2 ml-3 ml-4 ml-5 justify-between bg-white grid justify-center my-0 my-1 my-2 my-3 my-4 text-left justify-evenly  justify-center mx-5 mx-6 mx-7 mx-8 mx-9 mx-10 my-8 my-9 my-10 my-7 my-6 my-5 my-4 my-3 my-2 my-1 my-0  justify-around text-xl text-2xl my-2 my-3 my-4 my-5 my-6 my-7 text-sm m-5 m-4 bg-navyBlue text-white m-2 font-light object-cover object-contain object-fill object-none object-scale-down my-5 rounded-full m-auto w-7/12 font-bold w-44 w-4/12 mr-auto w-60 w-80 w-72 w-64 w-96 w-8/12 ml-auto overflow-auto p-3 border-2 border-4 border-r-4 border-black bg-blue-300 p-3 border-2 border-blue-600 h-screen not-prose relative bg-slate-50 rounded-xl overflow-hidden dark:bg-slate-800/25 relative rounded-xl overflow-auto overscroll-contain w-20 relative m-44 p-1.5 sticky fixed top-0 left-0 right-0 hidden bg-blue-600 h-screen text-center text-3xl">
        123
      </p>
      <p className="bg-gray-950 font-TSSemi font-TSbold font-TSmedium font-TSthin font-TSlight font-Acme fill-pink text-pink bg-pink border-pink font-TSExtra hover:-translate-x-2 lg:w-10/12 2xl:w-12/12 grid justify-between inline-block inline hover:scale-125 hover:scale-t-110 hover:scale-y-110 -translate-y-6 hover:-translate-y-2 hover:translate-x-2 text-slate950 text-gray-900 text-gray-800 text-navyBlue hidden">
        321
      </p>
      <p className="hidden bg-white bg-gray-100 text-gray-300 text-gray-400 text-gray-500 text-gray-600 text-gray-700 text-gray-800 text-gray-900 text-gray-200 text-gray-100">
        123
      </p>
      */}
      {/* Exp */}

      {/*
      {space}
       <section className="w-12/12 flex items-center md:justify-center">
        <div className={`text-black w-10/12 m-auto md:m-0 lg:w-6/12`}>
          {experienceData.map((item, idx) => {
            return (
              <div
                id="card"
                key={idx}
                className={`bg-white hover:translate-x-2 duration-700 ease-in-out ring-1 rounded-md my-5 p-0 md:p-4`}
              >
                <div className="flex items-center">
                  <Image
                    src={item.img}
                    className="hover:scale-105 duration-500 object-cover w-5/12 md:w-2/12 lg:w-2/12 2xl:w-2/12 rounded-tl-md md:rounded-md"
                    alt={item.Company_name}
                    quality={100}
                    width={400}
                    height={200}
                    loading="lazy"
                    placeholder="blur"
                    blurDataURL={item.img}
                  />
                  <div className="px-2 w-full">
                    <p
                      className={`text-black md:mt-4 text-base md:text-xl font-TSSemi`}
                    >
                      {item.title}
                    </p>
                    <div className="font-TSmedium mt-0 md:mt-2 grid md:flex 2xl:text-base lg:text-xs md:text-sm text-xs">
                      <p className="px-0.5 md:px-1">{item.Company_name}</p>

                      <p className="px-0.5 md:px-1">
                        {item.Employment_type}
                        <b className={`text-black px-0.5 md:px-1`}>|</b>
                        {item.Location_type}
                      </p>
                    </div>
                    <div className="font-TSmedium grid md:flex 2xl:text-base lg:text-xs md:text-sm text-xs">
                      <p className="px-0.5 md:px-1">{item.Location}</p>

                      <p className="px-0.5 md:px-1">
                        {item.start_date} <sub className="px-0.5">TO</sub>
                        {item.end_date}
                      </p>
                    </div>
                  </div>
                </div>
                <p className="font-TSmedium px-2 mt-0 md:mt-4 py-3 md:py-0 2xl:text-base lg:text-xs md:text-base text-sm">
                  <b className={`text-blue-800`}>Skills : </b>
                  {item.skills}
                </p>
              </div>
            );
          })}
        </div>
        <div className="hidden lg:grid w-4/12 m-auto">
          <Image
            src="/assest/img/experience.webp"
            className="hover:scale-105 duration-500 object-cover w-12/12 rounded-md m-auto"
            alt=""
            quality={100}
            width={600}
            height={400}
            loading="lazy"
            placeholder="blur"
            blurDataURL="/assest/img/experience.webp"
          />
        </div>
      </section>
      <hr className="mx-5 md:mx-0 lg:mr-16 w-12/12 m-auto border-b-2 opacity-20 border-gray-500 my-8" />
       */}
      {/* Footer  */}
      {/* <p className={`text-black bg-white font-TSlight pb-3 lg:pb-0 2xl:pb-0 text-center text-xs md:text-base`}>
       2023 MHMAD WREKAT
        <span className={`border-blue-200 border-l-4 mx-2`}>
          <span className="mx-2">Inc. All Rights Reserved</span>
        </span>
      </p> */}
      {/* Comment  */}
      {/* <section
        className={`text-blue-500 m-5 md:m-0 grid md:flex py-8 md:justify-around`}
      >
        <div className="md:px-4 w-11/12 md:w-6/12 lg:w-5/12 2xl:w-5/12 m-auto">
          <div
            className={`ring-1 bg-black flex flex-col w-full min-w-0 mb-6 break-words rounded-lg shadow-xl`}
          >
            <div className="flex-auto p-5 lg:p-10">
              <form
                id="feedbackForm"
                method="post"
                // onSubmit={handelFeedback}
                // ref={form}
              >
                <div className="w-full mb-3 font-TSbold">
                  <label
                    className="block mb-1 text-sm md:text-base font-bold uppercase"
                    htmlFor="name"
                  >
                    Name
                  </label>
                  <input
                    type="text"
                    name="name"
                    id="name"
                    className={`ring-1 bg-red-900 border-1 px-3 py-3 rounded text-sm shadow-lg w-full placeholder-black outline-none`}
                    placeholder=" "
                    required
                  />
                </div>
                <div className="w-full mb-3 font-TSbold">
                  <label
                    className="block mb-1 pt-2 text-sm md:text-base font-bold uppercase"
                    htmlFor="email"
                  >
                    Email
                  </label>
                  <input
                    type="email"
                    name="email"
                    id="email"
                    className={`ring-1 bg-white border-1 border-Green1 px-3 py-3 rounded text-sm  shadow-lg w-full placeholder-black outline-none`}
                    placeholder=" "
                    required
                  />
                </div>
                <div className="w-full mb-3 font-TSbold">
                  <label
                    className="block mb-1 pt-2 text-sm md:text-base font-bold uppercase"
                    htmlFor="feedback"
                  >
                    Message
                  </label>
                  <textarea
                    maxLength="300"
                    name="message"
                    id="feedback"
                    rows="4"
                    cols="80"
                    className={`ring-1 bg-white w-full px-3 py-3 text-sm placeholder-black border-1 rounded  shadow-lg focus:outline-none`}
                    placeholder=""
                    required
                  ></textarea>
                </div>
                <div className="mt-6 text-center">
                  <button
                    id="feedbackBtn"
                    type="submit"
                    className={`bg-white hover:scale-110 duration-500 ease-in-out text-gray-900 px-8 md:px-6 py-2 md:py-3 mx-auto mb-1 mr-1 text-sm font-bold text-center uppercase rounded shadow-lg outline-none focus:outline-none`}
                  >
                    Submit
                  </button>
                </div>
              </form>
            </div>
          </div>
        </div>
        <div className="md:pr-4 w-12/12 md:w-5/12">
          <p className={`text-black text-3xl font-TSbold pb-3`}>
            Most Comments
          </p>
          {commentsData.map((item, idx) => {
            return (
              <div
                key={idx}
                className={`bg-white hover:-translate-x-2 duration-700 my-4 overflow-hidden relative max-w-sm ring-1 rounded-xl flex items-center gap-6`}
              >
                <Image
                  src="/assest/img/user.webp"
                  className="absolute -left-6 w-24 h-24 rounded-full shadow-lg"
                  alt="mhmad wrekat image"
                  quality={100}
                  width={100}
                  height={50}
                  loading="lazy"
                  placeholder="blur"
                  blurDataURL="/assest/img/user.webp"
                />
                <div className="flex flex-col py-3 md:py-5 pl-20 md:pl-24">
                  <strong className={`text-base md:text-lg font-TSbold`}>
                    {item.name}
                  </strong>
                  <span
                    className={`text-black pl-2 text-sm font-TSmedium`}
                  >
                    {item.message}
                  </span>
                </div>
              </div>
            );
          })}
        </div>
      </section> */}
      {/* Project  */}
      {/* <section
          className={`bg-black w-12/12 md:w-11/12 md:mr-auto py-5`}
        >
          <section className="overflow-x-scroll flex">
            {projectsData.map((item, idx) => {
              return (
                <div key={idx} className="flex-none pb-10 pt-1 px-4">
                  <div
                    id="card"
                    className={`hover:-translate-y-2 duration-700 ease-in-out flex bg-gray-700 ring-1 rounded max-w-sm md:max-w-lg`}
                  >
                    <Image
                      src={item.img}
                      className="cursor-pointer object-cover w-4/12"
                      alt={item.name}
                      quality={100}
                      width={200}
                      height={100}
                      loading="lazy"
                      placeholder="blur"
                      blurDataURL={item.img}

                    />
                    <div className="grid justify-center p-4">
                      <p className="mb-2 text-2xl font-TSExtra">{item.name}</p>
                      <p className={`text-white mb-3 font-TSmedium`}>
                       desc
                      </p>
                      <b className={`bg-black w-5/12 text-xs py-1 text-gray-900 rounded text-center`}>
                    status
                      </b>
                    </div>
                  </div>
                </div>
              );
            })}
          </section>
        </section> */}
      {/* Skills  */}
      {/* <section
        className={`text-black font-TSbold justify-items-center md:justify-items-start grid m-auto grid-cols-2 gap-3 w-12/12 lg:w-11/12 lg:m-1 md:grid-cols-4 md:gap-5 md:mt-14 lg:mt-14 2xl:mt-14 mt-10`}
      >
        {programLanguage.map((item, idx) => {
          return (
            <div key={idx} className="rounded-lg flex items-center">
              <span className="hover:-rotate-12 duration-500 ease-in-out">
              ssvg
              </span>

              <p className="ml-1 md:ml-2 hover:-rotate-6 duration-500 ease-in-out font-bold text-sm md:text-lg lg:text-xl 2zl:text-lg">
              name
              </p>
            </div>
          );
        })}
      </section> */}
      {/* Head Line  */}
      {/* <section className="md:m-0 m-5 font-TSmedium">
        <div className="flex items-center">
          <div className={`bg-blue-200 w-1.5 h-9 rounded-full`} />
          <p className={`text-blue-900 text-3xl font-bold font-Acme ml-2`}>
            Lorem
          </p>
        </div>
        jsx
      </section> */}
      {/* Aside */}
      {/* <React.Fragment>
      <section
        className={`bg-gray-400 text-sky-500 fixed w-56 md:w-72 lg:w-56 2xl:w-72 text-center overflow-auto h-screen p-2`}
      >
        <div
          className={`w-2/12 ml-auto bg-gray-900 grid lg:hidden md:pl-1.5 pl-0.5 rounded`}
        >
          <button
            variant="text"
            color="blue-gray"
            className="p-2 z-10"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              strokeWidth={4}
              width={15}
              height={15}
              stroke="currentColor"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M6 18L18 6M6 6l12 12"
              />
            </svg>
          </button>
        </div>
        <p className="text-2xl font-Acme font-bold lg:my-5 my-2 mb-3">
          Mhmad Wrekat
        </p>
        <div className="w-7/12 m-auto">
          <Image
            src="/assest/img/mhmadwrekat.webp"
            className="rounded-full object-cover"
            alt="mhmad wrekat image"
            quality={100}
            width={500}
            height={600}
            loading="eager"
            priority
            placeholder="blur"
            blurDataURL="/assest/img/mhmadwrekat.webp"
          />
        </div>
        <p className="my-4 p-1.5 px-4 text-sm font-TSmedium">
          Hello, my name is Mohammad Wrekat and i am a Full Stack Developer.
          <br />
          Welcome to my personal website!
        </p>
        <div className="flex justify-evenly mx-6 mt-5">
          {socialMedia.map((item, idx) => {
            return (
              <div
                key={idx}

              >
svg              </div>
            );
          })}
        </div>
        <hr className="w-5/5 border-b-2 opacity-20 border-gray-500 my-8" />
        <div className="grid justify-center font-TSbold mb-10">
          {arrayOfTitles.map((item, idx) => {
            return (
              <a
                key={idx}
                className="flex hover:scale-110 duration-500 ease-in-out cursor-pointer hover:underline"

              >
               svg
                <p className="mb-3 pt-0.5 ml-3 text-left">text</p>
              </a>
            );
          })}
        </div>
        <hr className="w-5/5 border-b-2 opacity-20 border-gray-500 my-5" />
        <p className="mb-4 ml-3 font-TSbold">Dark / Lite Mode</p>

        <button
          className="text-2xl hover:scale-125 duration-500 ease-in-out"

        >
         buttom
        </button>
      </section>
    </React.Fragment> */}
      {/* Aside Mobile  */}
      {/* <React.Fragment>
      <section
        className={`bg-blue-900 flex justify-between items-center w-12/12 md:px-10 z-0`}
      >
        <button
          aria-label="menu"
          className="px-4 py-2 m-4"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            x="0px"
            y="0px"
            width={25}
            height={25}
            viewBox="0 0 50 50"
          >
            <path d="M 0 7.5 L 0 12.5 L 50 12.5 L 50 7.5 Z M 0 22.5 L 0 27.5 L 50 27.5 L 50 22.5 Z M 0 37.5 L 0 42.5 L 50 42.5 L 50 37.5 Z"></path>
          </svg>
        </button>
        <b className={`text-black font-Acme px-4 text-xl`}>
        time
        </b>
      </section>
      <section>
        {open ? (
          <section className="h-screen fixed top-0 z-40">
          <>5</>
          </section>
        ) : (
          <React.Fragment />
        )}
      </section>
    </React.Fragment> */}
      {/* About Me */}
      {/* <React.Fragment>
      <section
        id="aboutMe"
        className="font-TSmedium md:flex md:justify-start lg:flex lg:justify-start justifuy-between items-center"
      >
        <div className="w-12/12 md:w-7/12 lg:m-0 m-5">
          <div className="flex">
            <div className={`bg-gray-900 w-1.5 h-9 rounded-full`} />

            <h1
              className={`ml-2 text-3xl md:text-4xl font-bold font-Acme text-gray-200`}
            >
              Mhmad Wrekat
            </h1>
          </div>
          <h2
            className={`text-gray-300 text-start text-lg md:text-xl mt-2`}
          >
            Full Stack Developer | MERN
          </h2>

          <p
            className={`text-gray-400 text-base mt-5 md:mt-10 w-12/12 md:w-10/12`}
          >
            I Have a <b className={`text-gray-300`}>Bachelor`s Degree</b>{" "}
            in Software Engineering , and Full-Stack Web Development course from
            Code Fellows, Also MERN Stack course From Webtoon. <br />
            Specialised in frontend and backend development for web apps.
          </p>
          <div>
            <button
              className={`bg-gray-800 hover:scale-110 duration-500 ease-in-out text-gray-900 font-TSbold rounded mt-6`}
            >
              <a
                className="flex px-5 py-1.5"
                href="./assest/data/wrekat2023.pdf"
                download="wrekat2023.pdf"
              >
                <svg
                  className="h-3 w-3 lg:w-4 lg:h-4 mr-2 mt-1"
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 20 20"
                  fill="#111827"
                >
                  <path d="M13 8V2H7v6H2l8 8 8-8h-5zM0 18h20v2H0v-2z" />
                </svg>
                <b className="pt-0.5">View Resume</b>
              </a>
            </button>
          </div>
        </div>
        <div className="flex justify-center pt-5 w-10/12 md:w-8/12 m-auto">
          <Image
            src="/assest/img/mohammadWrekat.webp"
            className="md:max-w-xs rounded object-cover m-auto lg:m-0"
            alt="mhmad wrekat image"
            quality={100}
            width={400}
            height={100}
            loading="eager"
            priority
            placeholder="blur"
            blurDataURL="/assest/img/mohammadWrekat.webp"
          />
        </div>
      </section>
      <hr className="mx-5 md:mx-0 lg:mr-16 w-12/12 m-auto border-b-2 opacity-20 border-gray-500 my-8" />
    </React.Fragment> */}
      {/* Home */}
      {/* <React.Fragment>


        <div className={`lg:flex bg-gray-900`}>
          <div className="mr-auto 2xl:flex lg:flex hidden">

          </div>
          <div className="grid lg:hidden">

          </div>
          <div className="2xl:flex lg:flex hidden 2xl:px-44 lg:px-32" />

          <div className="lg:py-4 py-0 md:w-11/12 lg:w-10/12 2xl:w-11/12 md:m-auto lg:ml-auto 2xl:ml-auto">

          </div>
        </div>
      </React.Fragment> */}
      <Home />
    </React.Fragment>
  );
};

export default index;
// #020617 -->
